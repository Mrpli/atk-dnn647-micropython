...
function ci_stm32_atk_build {
    # Build ATK-DNN647 board (STM32N657X0).

    make ${MAKEOPTS} -C mpy-cross
    make ${MAKEOPTS} -C ports/stm32 BOARD=ATK_DNN647 submodules
    make ${MAKEOPTS} -C ports/stm32 BOARD=ATK_DNN647
    make ${MAKEOPTS} -C ports/stm32/mboot BOARD=ATK_DNN647
}

function ci_stm32_nucleo_build {
    # Build NUCLEO-N657X0 (same STM32N6 family as ATK_DNN647, reference board).

    make ${MAKEOPTS} -C mpy-cross
    make ${MAKEOPTS} -C ports/stm32 BOARD=NUCLEO_N657X0 submodules
    make ${MAKEOPTS} -C ports/stm32 BOARD=NUCLEO_N657X0
    make ${MAKEOPTS} -C ports/stm32/mboot BOARD=NUCLEO_N657X0
}
